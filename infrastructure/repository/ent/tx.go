...
func (tx *Tx) init() {
	tx.SkuAction = NewSkuActionClient(tx.config)
	tx.SkuConfig = NewSkuConfigClient(tx.config)
	tx.SkuStatusCode = NewSkuStatusCodeClient(tx.config)
	tx.SkuTemplate = NewSkuTemplateClient(tx.config)
}
...
// applies a query, for example: SkuAction.QueryXXX(), the query will be executed
